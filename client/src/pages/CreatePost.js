...
const CreatePost = () => {
   ...
   if(form.prompt && form.photo){
    ...
    try {
        const response=await fetch('http://localhost:3008/api/v1/post',{
            method:"POST",
            headers:{
   ...
   const generateImage=async ()=>{
    if(form.prompt){
        try{
         ...
         const response =await fetch ('http://localhost:3008/api/v1/tochka',
         {
            method:"POST",
            headers:{
                'Content-Type' : "application/json"
 ...
 '>The Community Showcase </h1>
                <p className='mt-2 text-[#666e75] text-[14px] max-w[500px]'>
                   Сreate imaginative and visually stunning through   TOCHKA AI
